INSERT INTO
    ingredients (nom_ingredient, categorie)
VALUES ('Tomate', 'légume'),
    ('Oignon', 'légume'),
    ('Poitrine de poulet', 'Autre'),
    ('Huile de Tournesol', 'Autre'),
    ('Sel', 'Autre'),
    ('Poivre noir', 'Autre'),
    ('Basilic', 'légume'),
    ('Origan', 'légume'),
    ('Parmesan', 'Autre'),
    ('Pain', 'Autre'),
    ('Beurre', 'Autre'),
    ('Lait', 'Autre'),
    ('Sucre', 'Autre'),
    ('Farine', 'Autre'),
    ('Œufs', 'Autre'),
    ('Levure', 'Autre'),
    ('Chocolat', 'Autre'),
    ('Vanille', 'fruit'),
    ('Avocat', 'fruit'),
    ('Citron', 'fruit'),
    ('Concombre', 'légume'),
    ('Carotte', 'légume'),
    ('Poivron', 'légume'),
    ('Fromage', 'Autre'),
    ('Jambon', 'Autre'),
    ('Saumon', 'Autre'),
    ('Riz', 'Autre'),
    ('Pates', 'Autre'),
    ('Pommes de terre', 'légume'),
    ('Épinards', 'légume'),
    ('Champignons', 'légume'),
    ('Noix', 'fruit'),
    ('Amandes', 'fruit'),
    ('Miel', 'Autre'),
    ('Yaourt', 'Autre'),
    ('Crème fraîche', 'Autre'),
    ('Ciboulette', 'légume'),
    ('Persil', 'légume'),
    ('Coriandre', 'légume'),
    ('Thym', 'légume'),
    ('Romarin', 'légume'),
    ('Curry', 'Autre'),
    ('Paprika', 'légume'),
    ('Cumin', 'légume'),
    ('Gingembre', 'légume'),
    ('Cannelle', 'Autre'),
    ('Clou de girofle', 'Autre'),
    ('Piment de Cayenne', 'légume'),
    ('Noix de muscade', 'fruit'),
    ('Anis étoilé', 'fruit'),
    ('Fenouil', 'légume'),
    ('Cardamome', 'fruit'),
    ('Safran', 'Autre'),
    ('Curcuma', 'légume'),
    ('Moutarde', 'Autre'),
    (
        'Vinaigre balsamique',
        'Autre'
    ),
    ('Sauce soja', 'Autre'),
    ('Bouillon de poulet', 'Autre'),
    ('Bouillon de bœuf', 'Autre'),
    ('Pate à pizza', 'Autre'),
    ('Jambon de parme', 'Autre'),
    ('Pancakes', 'Autre'),
    ('Myrtille', 'fruit'),
    ('Chorizo', 'Autre'),
    ('Mozzarella', 'Autre'),
    ('Ricotta', 'Autre'),
    ('Feta', 'Autre'),
    ('Gorgonzola', 'Autre'),
    ('Roquefort', 'Autre'),
    ('Gruyère', 'Autre'),
    ('Comté', 'Autre'),
    ('Emmental', 'Autre'),
    ('Cantal', 'Autre'),
    ('Saint-Nectaire', 'Autre'),
    ('Reblochon', 'Autre'),
    ('Tomme de Savoie', 'Autre'),
    ('Brie', 'Autre'),
    ('Camembert', 'Autre'),
    ('Chèvre', 'Autre'),
    ('Pate feuilletée', 'Autre'),
    ('Pate brisée', 'Autre'),
    ('Pate sablée', 'Autre'),
    ('Crème pâtissière', 'Autre'),
    ('Chantilly', 'Autre'),
    ('Glace à la vanille', 'Autre'),
    ('Glace au chocolat', 'Autre'),
    ('Glace à la fraise', 'Autre'),
    ('Sorbet au citron', 'Autre'),
    (
        'Sorbet à la framboise',
        'Autre'
    ),
    ('Fraise', 'fruit'),
    ('Framboise', 'fruit'),
    ('Mûre', 'fruit'),
    ('Cerise', 'fruit'),
    ('Pomme', 'fruit'),
    ('Poire', 'fruit'),
    ('Banane', 'fruit'),
    ('Orange', 'fruit'),
    ('Mandarine', 'fruit'),
    ('Clémentine', 'fruit'),
    ('Pamplemousse', 'fruit'),
    ('Ananas', 'fruit'),
    ('Mangue', 'fruit'),
    ('Kiwi', 'fruit'),
    ('Noix de coco', 'fruit'),
    (
        'Fruit de la passion',
        'fruit'
    ),
    ('Grenade', 'fruit'),
    ('Raisin', 'fruit'),
    ('Creme anglaise', 'Autre'),
    ('Steak', 'Autre'),
    ('Filet de saumon', 'Autre'),
    ('Filet de cabillaud', 'Autre'),
    ('Moules', 'Autre'),
    ('Calamars', 'Autre'),
    ('Thon en conserve', 'Autre'),
    (
        'Sardines en conserve',
        'Autre'
    ),
    (
        'Maquereau en conserve',
        'Autre'
    ),
    ('Laitue', 'légume'),
    ('Roquette', 'légume'),
    ('Chou kale', 'légume'),
    ('Chou-fleur', 'légume'),
    ('Brocoli', 'légume'),
    ('Asperges', 'légume'),
    ('Haricots verts', 'légume'),
    ('Petits pois', 'légume'),
    ('Maïs', 'légume'),
    ('Betterave', 'légume'),
    ('Radis', 'légume'),
    ('Navet', 'légume'),
    ('Courgette', 'légume'),
    ('Aubergine', 'légume'),
    ('Citrouille', 'légume'),
    ('Potiron', 'légume'),
    ('Patate douce', 'légume'),
    ('Céleri', 'légume'),
    ('Fenouil cru', 'légume'),
    ('frites surgelées', 'Autre'),
    ('Herbe de provence', 'légume'),
    ('Steak vegetarien', 'Autre'),
    ('Salade', 'légume'),
    ('Pain de mie', 'Autre'),
    ('Creme liquide', 'Autre'),
    ('Gelatine', 'Autre'),
    ('Fruits rouges', 'fruit'),
    ('Poudre de cacao', 'Autre'),
    ('Chocolat Noir', 'Autre'),
    ('Chocolat Blanc', 'Autre'),
    ('Chocolat au Lait', 'Autre'),
    ('Noix de Pecans', 'fruit'),
    ('Bacon', 'Autre'),
    ('Cheddar', 'Autre'),
    ('Burrrata', 'Autre'),
    (
        'Aiguillettes de poulet',
        'Autre'
    ),
    ('Escalope de veau', 'Autre'),
    ('Cuisse de poulet', 'Autre'),
    (
        'Filet mignon de porc',
        'Autre'
    ),
    ('Poitrine de porc', 'Autre'),
    ('Saucisse', 'Autre'),
    ('Chipolata', 'Autre'),
    ('Merguez', 'Autre'),
    ('Curry vert', 'Autre'),
    ('Curry rouge', 'Autre'),
    ('Lait de coco', 'Autre'),
    ('Echalote', 'légume'),
    ('Sauce barbecue', 'Autre'),
    ('Tortillade de mais', 'Autre'),
    ('Quinoa', 'Autre'),
    ('Tomates cerise', 'légume'),
    ('Menthe', 'légume'),
    (
        'Boulette de pomme de terre',
        'Autre'
    ),
    ('Haricots', 'légume'),
    ('Mais', 'légume'),
    ('Toffu', 'Autre'),
    ('Sauce Tomate', 'légume'),
    ('Nouilles de riz', 'Autre'),
    ('Germe de soja', 'légume'),
    ('Pain Burger', 'Autre'),
    ('Pates à lasagne', 'Autre'),
    ('Bechamel', 'Autre'),
    ('Ail', 'légume'),
    (
        'Lentilles de corail',
        'légume'
    ),
    (
        'Bouillon de legumes',
        'Autre'
    ),
    ('Piment rouge', 'légume'),
    ('Crevettes', 'Autre'),
    ('Spaguetti', 'Autre');

INSERT INTO
    utilisateurs (
        nom_utilisateur,
        email,
        mot_de_passe,
        role
    )
VALUES (
        'seb',
        'dorssebastien',
        'password',
        'admin'
    ),
    (
        'margault',
        'margaultmons',
        'password',
        'admin'
    ),
    (
        'camille',
        'camgra',
        'password',
        'admin'
    ),
    (
        'test',
        'testtest',
        'password',
        'user'
    );

INSERT INTO
    recettes (
        titre,
        description,
        instructions,
        temps_preparation_minutes,
        temps_cuisson_minutes,
        nombre_personnes,
        path_image
    )
VALUES (
        'Avocado Toast',
        'Une tartine à l''avocat simple et délicieuse, parfaite pour le petit-déjeuner ou le brunch.',
        '1. Faites griller le pain. 2. Écrasez l''avocat dans un bol et assaisonnez avec du sel, du poivre et du jus de citron. 3. Étalez l''avocat sur le pain grillé et ajoutez des toppings au choix.',
        5,
        5,
        1,
        '/assets/images/avocado_toast.jpg'
    ),
    (
        'Pizza Regina',
        'Une délicieuse pizza garnie de sauce tomate, mozzarella, jambon et champignons.',
        '1. Préchauffez le four à 220°C. 2. Étalez la pâte à pizza sur une plaque de cuisson. 3. Étalez la sauce tomate sur la pâte, puis ajoutez la mozzarella, le jambon et les champignons. 4. Enfournez pendant 12-15 minutes jusqu''à ce que la pâte soit dorée.',
        20,
        15,
        2,
        '/assets/images/pizza_regina.jpg'
    ),
    (
        'Soupe froide de concombre',
        'Une soupe rafraîchissante à base de concombre, yaourt et menthe.',
        '1. Épluchez et coupez les concombres en morceaux. 2. Dans un mixeur, ajoutez les concombres, le yaourt, la menthe, le sel et le poivre. 3. Mixez jusqu''à obtenir une consistance lisse. 4. Réfrigérez pendant au moins 30 minutes avant de servir.',
        10,
        0,
        4,
        '/assets/images/soupe_concombre.jpg'
    ),
    (
        'Pancakes aux myrtilles',
        'Des pancakes moelleux garnis de myrtilles fraîches, parfaits pour le petit-déjeuner.',
        '1. Dans un bol, mélangez la farine, le sucre, la levure chimique et une pincée de sel. 2. Dans un autre bol, battez l''œuf, puis ajoutez le lait et le beurre fondu. 3. Incorporez les ingrédients secs aux ingrédients liquides jusqu''à obtenir une pâte homogène. 4. Ajoutez les myrtilles à la pâte. 5. Faites chauffer une poêle antiadhésive et versez des petites louches de pâte pour former les pancakes. Faites cuire jusqu''à ce que des bulles se forment à la surface, puis retournez-les et faites cuire l''autre côté jusqu''à ce qu''il soit doré.',
        10,
        5,
        4,
        '/assets/images/pancakes_myrtilles.jpg'
    ),
    (
        'Moelleux au chocolat',
        'Un dessert riche et fondant au chocolat, parfait pour les amateurs de chocolat.',
        '1. Préchauffez le four à 180°C. 2. Faites fondre le chocolat et le beurre au bain-marie. 3. Dans un bol, battez les œufs et le sucre jusqu''à ce que le mélange blanchisse. 4. Incorporez le mélange chocolat-beurre, puis ajoutez la farine. 5. Versez la pâte dans des moules individuels et enfournez pendant 10-12 minutes.',
        15,
        30,
        6,
        '/assets/images/moelleux_chocolat.jpg'
    );

INSERT INTO
    recette_ingredients (
        id_recette,
        id_ingredient,
        quantite,
        unite
    )
VALUES
    -- Avocado Toast (recette_id=1)
    (1, 19, '1', 'unité'), -- Avocat
    (1, 10, '2', 'tranches'), -- Pain
    (1, 5, '1', 'pincée'), -- Sel
    (1, 6, '1', 'pincée'), -- Poivre noir
    (1, 20, '0.5', 'unité'), -- Citron
    -- Pizza Regina (recette_id=2)
    (2, 61, '1', 'unité'), -- Pate à pizza
    (2, 183, '200', 'g'), -- Sauce Tomate (CORRIGÉ: ID 199 au lieu de 200)
    (2, 66, '125', 'g'), -- Mozzarella
    (2, 25, '2', 'tranches'), -- Jambon
    (2, 31, '50', 'g'), -- Champignons
    -- Soupe froide de concombre (recette_id=3)
    (3, 21, '1', 'unité'), -- Concombre
    (3, 35, '1', 'pot'), -- Yaourt
    (3, 180, '10', 'feuilles'), -- Menthe
    (3, 5, '1', 'pincée'), -- Sel
    (3, 6, '1', 'pincée'), -- Poivre noir
    -- Pancakes aux myrtilles (recette_id=4)
    (4, 14, '250', 'g'), -- Farine
    (4, 13, '50', 'g'), -- Sucre
    (4, 16, '1', 'sachet'), -- Levure
    (4, 5, '1', 'pincée'), -- Sel
    (4, 15, '2', 'unités'), -- Œufs
    (4, 12, '30', 'cl'), -- Lait
    (4, 11, '50', 'g'), -- Beurre
    (4, 64, '100', 'g'), -- Myrtille
    -- Moelleux au chocolat (recette_id=5)
    (5, 17, '200', 'g'), -- Chocolat
    (5, 11, '150', 'g'), -- Beurre
    (5, 15, '4', 'unités'), -- Œufs
    (5, 13, '100', 'g'), -- Sucre
    (5, 14, '50', 'g');
-- Farine